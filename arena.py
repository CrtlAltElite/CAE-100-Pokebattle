...
class Arena():
    ...
    def battle_loop(self):
        while self.master1.pokemon and self.master2.pokemon:
            ...
            # Hp Balance for the remaining pokemon
            ...
            if self.master1.pokemon and self.master2.pokemon:
                input_red("\t\t\t\t\t --Press Enter For Next Round--")
            else:
                input_green("\t\t\t\t\t --Press Enter--")
                # Game is over Show our Victory Screen
    ...
    def battle(self):
        ...
        for index in range(len(self.master1.pokemon)):
            if master2_index >= 0 and self.master1.pokemon[index].hit_points > 0 \
                ...
                clear_screen()
                start_hp_p1 = int(self.master1.pokemon[index].hit_points)
                start_hp_p2 = int(self.master2.pokemon[master2_index].hit_points)

                self.master1.pokemon[index].attack(self.master2.pokemon[master2_index])

                # Make Stuff happen

                # Player 1 OG HP and show the image
                self.master1.pokemon[index].display()
                print_yellow(figlet_format(f'\tHP {start_hp_p1}',font="doom"))
                time.sleep(2)
                clear_screen()

                # Player 2 OG HP and show the image
                ...
                ball = ascii_magic.from_url("https://res.cloudinary.com/cae67/image/upload/v1657666237/pokeball_closed_kqdsgd.png")
                ascii_magic.to_terminal(ball)
                time.sleep(1)
                clear_screen()

                ball = ascii_magic.from_url("https://res.cloudinary.com/cae67/image/upload/w_1000,ar_1:1,c_fill,g_auto,e_art:hokusai/v1657666100/pokeballopen_rtossk.png ")
                ascii_magic.to_terminal(ball)
                time.sleep(1)
                clear_screen()

                # display both of the pokemon and show the damage taken
                ...
                print_yellow(figlet_format(str(damage_taken_p1) if damage_taken_p1 else 'invicible', font="doom"))

                self.master2.pokemon[master2_index].display(cols=40)
                print_blue(figlet_format(str(damage_taken_p2) if damage_taken_p2 else 'invicible', font="doom"))
                time.sleep(2)
